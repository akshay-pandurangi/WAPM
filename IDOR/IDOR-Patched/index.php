...
if(isset($_POST['submit'])){
 ...
 else
 {
    ...
    if($rows == 1){
        $_SESSION['idor_token'] = base64_encode(openssl_random_pseudo_bytes(32));
        $token = $_SESSION['idor_token'];
        $query = mysqli_query($conn, "UPDATE voterlist SET token = '$token' WHERE votername='$user' AND password='$pass'");
        $_SESSION['user'] = $user;
        $_SESSION['pass'] = $pass;
